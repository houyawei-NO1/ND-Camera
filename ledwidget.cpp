...
LedWidget::LedWidget(QWidget *parent) : QWidget(parent)
{
    ...
    led_pic->setStyleSheet("background-color:rgb(43,48,70);");
    led_pic->setPixmap(QPixmap::fromImage(image));
    led_pic->resize(QSize(image.width()*0.1,image.height()));
    // led_pic->setScaledContents(true);//图片自适应窗口

    m_text_on = new  QLabel(this);
    m_text_on->setText("开信号触发");
//    m_text_on->setFont(font);
//    m_text_on->setPalette(pe);
    m_text_on->setStyleSheet("background-color:rgb(43,48,70);color:rgb(255,245,238);font:13px");
    m_color_on = new  CDoubleButton(this);
    m_color_on->setFixedSize(18,18);
...
//    m_text_off->setPalette(pe);
    m_text_off->setStyleSheet("background-color:rgb(43,48,70);color:rgb(255,245,238);font:13px");
    m_color_off = new  CDoubleButton(this);
    m_color_off->setFixedSize(18,18);
...
//    m_text_final->setPalette(pe);
    m_text_final->setStyleSheet("background-color:rgb(43,48,70);color:rgb(255,245,238);font:13px");
    m_color_final = new  CDoubleButton(this);
    m_color_final->setFixedSize(18,18);
    ...
    led_name->setStyleSheet("background-color:rgb(43,48,70);color:white;font:20px");
...
//    led_layout->addStretch();
    ...
    setLayout(pic_layout);

}

void LedWidget::Init(QString str_name)
 {
     m_name  = str_name;
...
//    led_pic->resize(QSize(image.width(),image.height()));
     ...
     off_flag = false;
     m_result = 0;
     m_color_on->setStyleSheet("background-color:rgb(43,48,70);border-radius:8px;background-color:rgb(255,245,238)");
     m_color_off->setStyleSheet("background-color:rgb(43,48,70);border-radius:8px;background-color:rgb(255,245,238)");
     m_color_final->setStyleSheet("background-color:rgb(43,48,70);border-radius:8px;background-color:rgb(255,245,238)");
}
void LedWidget:: LedChange(bool status)
 {
     QImage image;
      if(status)
      {
